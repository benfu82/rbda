...
public class Segment {
	...
	public Segment(String name, float start, float end, int pipeID) {
		this(-1, pipeID, name, start, end, true);
	}

	private Segment(int id, int pipeID, String name, float start, float end,
			boolean isNew) {
		this.id = id;
		this.pipeID = pipeID;
		this.name = name;
	...
	public static Segment[] getAll(DBQueryer queryer, int pipeID) {
		String sql = String.format("select * from %s where pipe_id=%d",
				TABLE_NAME, pipeID);
		ResultSet rs = queryer.query(sql);
		if (rs == null)
			return null;
		Collection<Segment> list = new ArrayList<Segment>();
		try {
			while (rs.next()) {
				int id = rs.getInt("id");
				...
				list.add(new Segment(id, pipeID, name, start, end, false));
			}
	...
	public Segment clone() {
		return new Segment(this.id, this.pipeID, this.name, this.start,
				this.end, this.isNew);
	}
